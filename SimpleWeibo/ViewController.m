...
//    obj = self;
    ...
    UIButton *statussButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [statussButton setTitle:NSLocalizedString(@"status", nil) forState:UIControlStateNormal];
...
- (void)testRequestForGetStatusIDs
{
    ...
    [WBHttpRequest requestForStatusIDsFromCurrentUser:[DataHolder sharedInstance].wbCurrentUserID withAccessToken:[DataHolder sharedInstance].wbtoken andOtherProperties:nil queue:nil withCompletionHandler:^(WBHttpRequest *httpRequest, id result, NSError *error) {
...
- (void) testRequestForStatus {
      __weak ViewController *blocksafeSelf = self;
    [self getStatusesWithSinceId:0 maxId:0 count:20 page:1 feature:0 trimUser:0 success:^ (BOOL isSuccess, NSMutableArray *array) {
...
void DemoRequestHanlder(WBHttpRequest *httpRequest, id result, NSError *error, ViewController* controller)
{
    NSString *title = nil;
...
//          NSLog(@"weatherDic is %@", weatherDic);
        ...
        BYUsersCellsViewController *first = [[BYUsersCellsViewController alloc]init];
        first.status = array;
        [controller.navigationController pushViewController:first animated:YES];
